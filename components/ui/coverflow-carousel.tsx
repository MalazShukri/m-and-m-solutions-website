...
export interface CoverflowCarouselProps {
  ...
  /** Fires whenever the centred slide changes — lets a parent render its own
      detail panel (links, badges, etc.) instead of relying on the built-in caption. */
  onSelect?: (index: number) => void;
  /** Externally jump to a slide (e.g. from a parent-rendered thumbnail rail). */
  activeIndex?: number;
}
...
export function CoverflowCarousel({
  ...
  const setSelected = React.useCallback((index: number) => {
    setSelectedState((prev) => {
      if (prev !== index) onSelectRef.current?.(index);
      return index;
    });
  }, []);
  ...
  // Let a parent drive the selection (e.g. clicking a thumbnail elsewhere on
  // the page) without fighting the drag/settle state machine above.
  const lastExternalIndex = React.useRef<number | undefined>(undefined);
  React.useEffect(() => {
    if (activeIndex === undefined) return;
    if (activeIndex === lastExternalIndex.current) return;
    lastExternalIndex.current = activeIndex;
    if (indexAt(targetRef.current) !== activeIndex) goTo(activeIndex);
